...
- 修复风速/温度传感器读取：按冠拓电子说明书解析，0x03 帧读风速，0x05 厂家自定义帧读温度。
...
| `wind` | 风速单变量实验 | 基础实验 | W_s, W_t | 温度关于风速变化 |
...
起始寄存器: 0x0000
寄存器数量: 2
请求帧: 04 05 00 00 00 02 4C 5E
返回: W_t
W_t = (register[1] - 400) / 10  (C)
```
...
风速/温度传感器当前按两个厂家帧解析，两个响应的第一个寄存器保留不用：

```text
0x03 响应: W_s = register[1] / 10
0x05 响应: W_t = (register[1] - 400) / 10  (C)
```
