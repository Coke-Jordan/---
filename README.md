...
7. 点击 `清空本轮数据` 开始正式记录。
8. 调整实验工况，观察曲线和关系图随数据动态演变。
9. 需要固定局部细节时直接缩放或拖拽图表；需要回到最新范围时点击 `重置当前视图`。
10. 需要拟合时设置阶数并点击 `计算拟合`。
11. 点击 `导出 CSV` 保存数据。
12. 实验结束后点击 `停止采集`。
...
- 先完成缩放、拖拽或单图切换，需要恢复全局范围时再点击 `重置当前视图`。
- 避免同时打开多个同一实验页面采集。
